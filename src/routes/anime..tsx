...
import { getDisplayTitle } from '@/lib/utils/animeTitles'
...
async function getAnimeInfo(id: number) {
    ...
    // 🔥 OBTENER EL TÍTULO LEGIBLE PARA MOSTRAR EN LA UI
            ...
            <div className="container mx-auto px-4 py-8">
                {anime.bannerImage && (
                    <div className="relative h-64 md:h-80 rounded-lg overflow-hidden mb-8">
                        <img src={anime.bannerImage} alt={anime.displayTitle} className="w-full h-full object-cover" />
                        <div className="absolute inset-0 bg-gradient-to-t from-black via-black/50 to-transparent"></div>
                    </div>
                )}
                <div className="flex flex-col md:flex-row gap-8">
                    <div className="md:w-1/4 flex-shrink-0">
                        <img src={anime.image} alt={anime.displayTitle} className="w-full rounded-lg shadow-lg" />
                    </div>
                    <div className="md:w-3/4">
                        <h1 className="text-3xl font-bold mb-2">{anime.displayTitle}</h1>
                        {anime.description && (
